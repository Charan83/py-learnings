...
#           - using generator functions(a way to create generator which is an iterator) which use yield keyword
#               - uses yield
...
def count_up_to(max):
    ...
    while count <= max:
        ...
        # yield will keep the most recent val in the memory(not all values are kep in the memory), retun the val when next is called on the generator obj and pause until the next time it is called again
        yield count
        count += 1
...
# generator_object() we can't execute a generator functio as its not callable, its an object (generator object)
...
#   Note: generator function return generator which is an iterator
